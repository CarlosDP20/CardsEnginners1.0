document.addEventListener("DOMContentLoaded", function() {
  ...
  form.addEventListener("submit", function(event) {
    ...
    const newCard = { name, lastname, country, skills };
    if (editIndex !== null) {
  ...
  function renderCards(cardsToRender) {
    ...
    cardsToRender.forEach((card, index) => {
      ...
      cardsByCountry[card.country].push({ card, index });
    });
        ...
        cardElement.innerHTML = `
          <div class="card-content">
            ...
            ${hiddenSkills.length > 0 ? `
              <button class="toggle-skills-btn">Mostrar más habilidades (${hiddenSkills.length})</button>
  ...
  function renderSkill(skill) {
    // Verifica que el rating esté dentro del rango esperado
    const rating = Math.max(0, Math.min(skill.rating, 10)); 
    const progressColor = rating <= 3 ? 'progress-red' :
                          rating <= 7 ? 'progress-yellow' : 'progress-green';
    const widthPercentage = rating * 10; // Convierte el rating en porcentaje para el ancho
    return `
      <li>
        <div class="progress-bar">
          <div class="${progressColor}" style="width: ${widthPercentage}%; min-width: 30px;">
            ${skill.name} (${rating}/10)
          </div>
        </div>
      </li>`;
  }
  ...
  function attachEditCardListeners() {
    ...
    editCardBtns.forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        const card = cards.find(c => c.name === cardElement.querySelector('h2').textContent);
